...
class dataParser:
    ...
    def __init__(self, GUI, configFile):
        ...
        data_groups = [x.strip() for x in getattr(self, "general").getSetting("data_chips").split(',')]
        for group in data_groups:
            setattr(self, group + "DecimalDict", defaultdict(list))
            setattr(self, group + "BinaryDict", defaultdict(list))
            for channel in [x.strip() for x in getattr(self, group).getSetting("data_channels").split(',')]:
