...
class AnalyzePed(object):
    ...
    def PlotRaw(self,plot_dir,meas_to_plot = None, gains_to_plot = None, chans_to_plot = None):
        ...
        if not meas_to_plot:  meas_to_plot = [value for value in list(range(self[0].nMeas)) if value in list(range(self[1].nMeas))]
        if not gains_to_plot: gains_to_plot = [value for value in list(range(self[0].Gains)) if value in list(range(self[1].Gains))]
        if not chans_to_plot: chans_to_plot = [value for value in list(range(self[0].Channels)) if value in list(range(self[1].Channels))]
    ...
    def AnalyzeBaseline(self,plot_dir, runName, meas_to_plot = None, gains_to_plot = None, chans_to_plot = None):
        ...
        for meas in meas_to_plot:
          for i,gain in enumerate(gains_to_plot):
            for j,channel in enumerate(chans_to_plot):
                ...
                mu, sigma, dsig, rms = self.makeFittedHist(pedestal,plot_dir,"Baseline value, Ped Run",channel,gain,plot=0)
        ...
        mdac_out = [mdac_lo,mdac_hi,"MDAC",plot_dir]
        if len(dre_hi) == 0 or len(dre_lo) == 0: 

            print("No DRE channels to analyze"); 
            return
	#self.PlotSummary(dre_lo,dre_hi,"DRE",plot_dir)
        dre_out = [dre_lo,dre_hi,"DRE",plot_dir]
        return(mdac_out,dre_out)


    def PlotSummary(self,data_1,data_2,chtype, plot_dir, gain):

        fig, ax = plt.subplots(1)                
        plt.xticks(rotation = 45)
        fig2, ax2 = plt.subplots(1)                
        for col,title,data in [('b',self[1].runNo,data_1), ('r',self[2].runNo,data_2)] :

            names, mus, stds = zip(*data)
            ax.grid(zorder = 0)
            ax.bar(names,mus,fill = False,ec = col, label = title, zorder = 3) 
            ax.set_title(chtype + " Mean Pedestal Value" + gain)
            ax.set_ylabel("ADC Counts")
            ax.set_ylim(0,max(mus) + max(mus)/3)
            ax.legend()
            fig.savefig(r'{plot_dir}/{chtype}_mu_summary_comp.png'.format(plot_dir = plot_dir,chtype = chtype) )
            
            names, mus, stds = zip(*data)
            ax2.grid(zorder = 0)
            plt.xticks(rotation = 45)
            ax2.bar(names,stds,fill = False,ec = col, label = title, zorder = 3) 
            ax2.set_title(chtype + " Pedestal RMS" + gain)
            ax2.set_ylabel("ADC Counts")
            ax2.set_ylim(0,max(stds) + max(stds)/4)
            ax2.legend()
            fig2.savefig(r'{plot_dir}/{chtype}_rms_summary_comp.png'.format(plot_dir = plot_dir,chtype = chtype) )
...
def PlotSummary(runs_to_plot,run_data,chtype, plot_dir, gain):
    ...
    if len(runs_to_plot) > 3:
        colors.append(np.random.rand(len(runs_to_plot)-3,3))
    else:
        colors = colors[:len(runs_to_plot)]
    plotting_data = list(zip(colors,runs_to_plot,run_data))
    #for col,title,data in [('b',runs_to_plot[0],data_1), ('r',runs_to_plot[1],data_2)] :
    for col,title,data in plotting_data:

        names, mus, stds = zip(*data)
        names = [name[:2] + name[7:] for name in names] # very janky method to change 'channelxxx' --> 'chxxx' in bar labels
        ax.grid(zorder = 0, b=True)
        ax.bar(names,mus,fill = False,ec = col, label = title, zorder = 3) 
        #ax.bar(names,mus,fill = False, label = title, zorder = 3) 
        ax.set_title(chtype + " Mean Pedestal Value" + gain)
        ax.set_ylabel("ADC Counts")
        ax.set_ylim(0,max(mus) + max(mus)/3)
        ax.legend()
        fig.savefig(r'{plot_dir}/{chtype}_{gain}_mu_summary_comp.png'.format(plot_dir = plot_dir,chtype = chtype, gain = gain) )
        
        names, mus, stds = zip(*data)
        names = [name[:2] + name[7:] for name in names] # very janky method to change 'channelxxx' --> 'chxxx' in bar labels
        ax2.grid(zorder = 0, b=True)
        plt.xticks(rotation = 70)
        mean = np.mean(stds)
        ax2.bar(names,stds,fill = False,ec = col, label = "{} mean = {:.2f}".format(title,mean), zorder = 3) 
        #ax2.bar(names,stds,fill = False, label = title, zorder = 3) 
        ax2.set_title(chtype + " Pedestal RMS " + gain)
        ax2.set_ylabel("ADC Counts")
        ax2.set_ylim(0,max(stds) + max(stds)/4)
        print(title)
        ax2.legend()
        fig2.savefig(r'{plot_dir}/{chtype}_{gain}_rms_summary_comp.png'.format(plot_dir = plot_dir,chtype = chtype, gain= gain) )
...
def main():
    ...
    if len(sys.argv) < 3 :
        print("ERROR, program requires at least 2 runname arguments")
        return 

    n_runs = len(sys.argv) - 1
    runNames = sys.argv[1:]
   
    plot_dir = "../data/Processed/" + runNames[0] + "_" + runNames[-1] + "_" + str(len(runNames))
    ...
    runs_to_plot = []
    for run in runNames:
        input_dir = "../data/Processed/" + run + "/"
    
        PedData = AnalyzePed(input_dir + "Data_Normal.hdf5",run)
        ...
        print(dir(PedData))
        baseline_run = PedData.AnalyzeBaseline(plot_dir, run)
        baseline_runs.append(baseline_run)
        print(run)
        print(baseline_run)
        runs_to_plot.append(PedData.runNo)
    ...
    #baseline_run1 = PedData[0].AnalyzeBaseline(plot_dir, runName1)
    ##baseline_run2 = PedData[1].AnalyzeBaseline(plot_dir, runName2)
    #print(len(baseline_runs))
    ch_data = list(zip(*baseline_runs))
    #print(len(ch_data))
    for i in range(0,len(baseline_runs[0])):
        ch_type = baseline_runs[0][i][2]
        gain_data = list(zip(*ch_data[i]))
        #print(len(ch_data[i]))
        #print(len(gain_data))
        #print(len(gain_data[0]))
        for j in range(0,2):
            gain = ["LO", "HI"][j]
            print(ch_type)
            print(gain)
            #print(gain_data[j])
            #PlotSummary(runs_to_plot, baseline_run1[i][j], baseline_run2[i][j], ch_type, plot_dir, gain)
            PlotSummary(runs_to_plot, gain_data[j], ch_type, plot_dir, gain)
