...
def fuseLpGBT(port, lpgbtAddr, regAddr, data):
    ...
    # writeToUSBISS(port, writeMessage)
    # readFromUSBISS(port)
    readBack = readFromLpGBT(port, lpgbtAddr, FUSEValuesA, len(data))
    ...
    isError = False
    for num in range(len(readBack)) :
      if readBack[num] != data[num]:
        print("ERROR: readback EFUSE register values do not agree with write value")
        print("REGISTER",regAddr)
        print("DATA",data)
...
def main(pArgs):
    ...
    if pArgs.lpgbtNum == 11:
        lpgbtAddr = 0b1110001
    elif pArgs.lpgbtNum == 12:
        lpgbtAddr = 0b1110010
    elif pArgs.lpgbtNum == 13:
        lpgbtAddr = 0b1110011
    elif pArgs.lpgbtNum == 14:
        lpgbtAddr = 0b1110100
    else:
    ...
    regAddr  = [0x01c, 0x020, 0x024, 0x028, 0x02c, 0x030, 0x034, 0x038, 0xa8, 0xe8, 0x0ec]
    regDataA = [ 0x00,  0xc8,  0x55,  0x05,  0x88,  0x0a,  0x00,  0x00, 0x10, 0x07,  0x00]
    regDataB = [ 0x00,  0x38,  0x55,  0x1b,  0x89,  0x0a,  0x00,  0x20, 0x00, 0x00,  0x00]
    regDataC = [ 0x00,  0x44,  0x55,  0x00,  0x99,  0x0a,  0x00,  0x00, 0x00, 0x00,  0x00]
    regDataD = [ 0x55,  0x55,  0x55,  0x00,  0x0a,  0x00,  0x00,  0x00, 0x94, 0x00,  0x07]
